...
if($radio==1 && $usnvalid)
{
    mysql_query("insert into activity3 values(NULL,'$stext',NULL,NULL)") or die("query failed");
header("Location:success.xhtml");
}
else if($radio==2 && $usnvalid5 && $usnvalid6){
    mysql_query("insert into activity3 values(NULL,'$stext4','$gtext5','NULL')") or die("query failed");
header("Location:success.xhtml");
}

else if($radio==3 && $usnvalid2 && $usnvalid3 && $usnvalid4){
    mysql_query("insert into activity3 values(NULL,'$stext1','$gtext2','$gtext3')") or die("query failed");
header("Location:success.xhtml");
}
else{
